...
public class ClassGenerator {
    ...
    public static void generateFlatClass(GenerationContext ctx, String currentPackageDir, EPackage packElement, EClass cls) {
        ...
        pr.println("override internal var internal_inboundReferences : java.util.HashMap<String, MutableList<String>> = java.util.HashMap<String,  MutableList<String>>()");
        ...
        if (ctx.persistence) {
            ...
            for(EReference ref : cls.getEAllReferences()) {
                if (ref.isMany()) {
                    ...
                    pr.println("(" + ProcessorHelper.getInstance().protectReservedWords(ref.getName()) + "!! as " + ctx.kevoreeContainerImplFQN + ").removeInboundReference(path()!!," + ProcessorHelper.getInstance().fqn(ctx, ctx.basePackageForUtilitiesGeneration) + ".util.Constants.Ref_" + ref.getName() + ")");
    ...
    private static String generateInternalSetter(GenerationContext ctx, EClass cls, EReference ref, String typeRefName) {
        ...
        if (!ref.isMany()) {
            ...
            if (ref.isContainment()) {
                ...
                if (ref.isMany()) {
                    ...
                    if (ref.isRequired()) {
                        ...
                        res.append("(" + ref.getName() + param_suf + " as " + ctx.kevoreeContainerImplFQN + " ).setEContainer(this,null," + ProcessorHelper.getInstance().fqn(ctx, ctx.basePackageForUtilitiesGeneration) + ".util.Constants.Ref_" + ref.getName() + ")\n");
                    }
                }
                res.append("}\n");
            ...
            res.append("if(" + ref.getName() + param_suf + " != null) {\n");
            res.append("(" + ref.getName() + param_suf + "!! as " + ctx.kevoreeContainerImplFQN + ").addInboundReference(path()!!, " + ProcessorHelper.getInstance().fqn(ctx, ctx.basePackageForUtilitiesGeneration) + ".util.Constants.Ref_"+ref.getName()+")\n");
            res.append("} else {\n");
            res.append("($" + ProcessorHelper.getInstance().protectReservedWords(ref.getName()) + "!! as " + ctx.kevoreeContainerImplFQN + ").removeInboundReference(path()!!, " + ProcessorHelper.getInstance().fqn(ctx, ctx.basePackageForUtilitiesGeneration) + ".util.Constants.Ref_"+ref.getName()+")\n");
            res.append("}\n");
            ...
            res.append("(el as " + ctx.kevoreeContainerImplFQN + ").addInboundReference(path()!!, " + ProcessorHelper.getInstance().fqn(ctx, ctx.basePackageForUtilitiesGeneration) + ".util.Constants.Ref_"+ref.getName()+")\n");
    ...
    private static String generateRemoveMethodWithParam(EClass cls, EReference ref, String typeRefName, GenerationContext ctx) {
        ...
        res.append("(" + ref.getName() + param_suf + " as " + ctx.kevoreeContainerImplFQN + ").removeInboundReference(path()!!, " + ProcessorHelper.getInstance().fqn(ctx, ctx.basePackageForUtilitiesGeneration) + ".util.Constants.Ref_" + ref.getName() + ")\n");
        if (ref.isContainment()) {
            //TODO
        ...
        if (ctx.generateEvents) {
            if (ref.isContainment()) {
                ...
                res.append("fireModelEvent(org.kevoree.modeling.api.events.ModelEvent(path(), org.kevoree.modeling.api.util.ActionType.REMOVE, org.kevoree.modeling.api.util.ElementAttributeType.CONTAINMENT, " + ProcessorHelper.getInstance().fqn(ctx, ctx.basePackageForUtilitiesGeneration) + ".util.Constants.Ref_" + ref.getName() + ", " + ref.getName() + param_suf + ",previousPathToBeRemoved))\n");
                res.append("}\n");
            } else {
                res.append("if(fireEvents) {\n");
                res.append("fireModelEvent(org.kevoree.modeling.api.events.ModelEvent(path(), org.kevoree.modeling.api.util.ActionType.REMOVE, org.kevoree.modeling.api.util.ElementAttributeType.REFERENCE, " + ProcessorHelper.getInstance().fqn(ctx, ctx.basePackageForUtilitiesGeneration) + ".util.Constants.Ref_" + ref.getName() + ", " + ref.getName() + param_suf + ",previousPathToBeRemoved))\n");
    ...
    private static String generateRemoveAllMethodWithParam(EClass cls, EReference ref, String typeRefName, GenerationContext ctx) {
        ...
        if (ref.getEOpposite() != null) {
            if (ref.isContainment()) {
                ...
                res.append("(el as " + ctx.kevoreeContainerImplFQN + ").removeInboundReference(path()!!, " + ProcessorHelper.getInstance().fqn(ctx, ctx.basePackageForUtilitiesGeneration) + ".util.Constants.Ref_"+ref.getName()+")\n");
