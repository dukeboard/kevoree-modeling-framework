...
class ModelTraceApplicator {
  ...
  public applyTraceSequence(traceSeq: TraceSequence, callback: Callback<Throwable>): void {
    Helper.forall(traceSeq.traces(), 
      public on(modelTrace: ModelTrace, next: Callback<Throwable>): void {
  ...
  public applyTrace(trace: ModelTrace, callback: Callback<Throwable>): void {
    if (trace instanceof ModelAddTrace) {
      ...
      this.targetModel.view().lookup(trace.getSrcKID(), 
        public on(resolvedTarget: KObject): void {
      ...
      if (trace instanceof ModelRemoveTrace) {
          ...
          public on(targetElem: KObject): void {
            if (targetElem != null) {
              this.targetModel.view().lookup(removeTrace.getObjKID(), 
                public on(remoteObj: KObject): void {
        ...
        if (trace instanceof ModelSetTrace) {
          ...
          if (!trace.getSrcKID().equals(this.pendingObjKID)) {
            this.targetModel.view().lookup(trace.getSrcKID(), 
              public on(tempObject: KObject): void {
                if (tempObject == null) {
