...
class AbstractKObject<A extends KObject<any,any>, B extends KView> implements KObject<A, B> {
  ...
  private getCreateOrUpdatePayloadList(obj: KObject<any,any>, payloadIndex: number): any {
    ...
    if (previous == null) {
      if (payloadIndex == AbstractKObject.INBOUNDS_INDEX) {
        previous = new JUHashMap<number, number>();
      } else {
        previous = new JUHashSet<number>();
      }
  ...
  public mutate(actionType: KActionType, metaReference: MetaReference, param: KObject<any,any>, setOpposite: boolean): void {
    if (actionType.equals(KActionType.ADD)) {
      if (metaReference.single()) {
        ...
        var previousList: JUSet<number> = <JUSet<number>>this.getCreateOrUpdatePayloadList(this, metaReference.index());
        ...
        var inboundRefs: JUMap<number, number> = <JUMap<number, number>>this.getCreateOrUpdatePayloadList(param, AbstractKObject.INBOUNDS_INDEX);
          ...
          if (metaReference.single()) {
            ...
            if (previousKid != null) {
              ...
              if (resolvedParam != null) {
                ...
                var inboundRefs: JUMap<number, number> = <JUMap<number, number>>this.getCreateOrUpdatePayloadList(resolvedParam, AbstractKObject.INBOUNDS_INDEX);
            ...
            var previous: any = payload[metaReference.index()];
            if (previous != null) {
              var previousList: JUSet<number> = <JUSet<number>>previous;
              if (this.now() != this._now) {
                var previousListNew: JUSet<number> = new JUHashSet<number>();
  ...
  public each<C extends KObject<any,any>> (metaReference: MetaReference, callback: Callback<C>, end: Callback<Throwable>): void {
    ...
    if (o == null) {
      ...
      if (o instanceof JUSet) {
        var objs: JUSet<number> = <JUSet<number>>o;
  ...
  private internal_visit(visitor: ModelVisitor, end: Callback<Throwable>, deep: boolean, treeOnly: boolean, alreadyVisited: JUHashSet<number>): void {
    if (alreadyVisited != null) {
      alreadyVisited.add(this.uuid());
    }
    var toResolveds: JUSet<number> = new JUHashSet<number>();
    for (var i: number = 0; i < this.metaReferences().length; i++) {
  ...
  public graphVisit(visitor: ModelVisitor, end: Callback<Throwable>): void {
    this.internal_visit(visitor, end, true, false, new JUHashSet<number>());
  }
    ...
    for (var i: number = 0; i < this.metaReferences().length; i++) {
      ...
      if (payload != null) {
        ...
        if (this.metaReferences()[i].single()) {
          ...
          var elems: JUSet<number> = <JUSet<number>>payload;
  ...
  public traces(request: TraceRequest): ModelTrace[] {
    ...
    if (TraceRequest.REFERENCES_ONLY.equals(request) || TraceRequest.ATTRIBUTES_REFERENCES.equals(request)) {
      for (var i: number = 0; i < this.metaReferences().length; i++) {
        ...
        if (o instanceof JUSet) {
          var contents: JUSet<number> = <JUSet<number>>o;
    ...
    if (rawPayload == null) {
      ...
      if (payload != null) {
        if (payload instanceof JUMap) {
          var refs: JUMap<number, number> = <JUMap<number, number>>payload;
          var oppositeKids: JUSet<number> = new JUHashSet<number>();
