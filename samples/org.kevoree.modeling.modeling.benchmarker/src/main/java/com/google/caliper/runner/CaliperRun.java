...
public final class CaliperRun {
  ...
  public void run() throws InvalidBenchmarkException {
    ...
    for (int i = 0; i < options.trialsPerScenario(); i++) {
      for (Scenario scenario : scenariosToRun) {
        ...
        scenariosExecuted++;
        console.print(String.format("\r%d of %d measurements complete: %.1f%%.",
            scenariosExecuted, totalTrials, scenariosExecuted * 100.0 / totalTrials));
      }
    }
    console.print("\n");
  ...
  private List<ResultProcessor> createResultProcessors() {
    ...
    builder.add(WebappUploader.create(config.asCaliperRc()));
    return builder.build();
  }

  /**
   * Attempts to run each given scenario once, in the current VM. Returns a set of all of the
   * scenarios that didn't throw a {@link com.google.caliper.api.SkipThisScenarioException}.
   */
