...
package org.kevoree.tools.ecore.gencodeKotlin.serializer

import scala.collection.JavaConversions._
import java.io.{File, FileOutputStream, PrintWriter}
import org.kevoree.tools.ecore.gencodeKotlin.loader.RootLoader
...
class SerializerGenerator(ctx: GenerationContext) {
  ...
  private def generateDefaultSerializer(genDir: String, packageName: String, root: EClass, rootXmiPackage: EPackage, sub: Set[String]) {
    ...
    val pr = new PrintWriter(new File(genDir + "ModelSerializer.kt"), "utf-8")
