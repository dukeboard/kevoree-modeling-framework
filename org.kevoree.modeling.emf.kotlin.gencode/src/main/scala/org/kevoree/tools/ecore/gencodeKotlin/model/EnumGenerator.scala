...
trait EnumGenerator {
  ...
  def generateEnum(ctx : GenerationContext, currentPackageDir: String, packElement: EPackage, en: EEnum) {
    ...
    val pr = new PrintWriter(new File(currentPackageDir + "/" + formattedEnumName + ".kt"), "utf-8")
    ...
    en.getELiterals.foreach {
      enumLit =>
        pr.println("case object " + enumLit.getName.toUpperCase + " : " + formattedEnumName + " { val name = \"" + enumLit.getName + "\"; val value = " + enumLit.getValue + " }")
    }
    pr.println("case class Unknown" + formattedEnumName + "(name: String, value: Int) : " + formattedEnumName)
    pr.flush()
    pr.close()
  }
