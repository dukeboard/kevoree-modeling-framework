...
trait TraitGenerator {
  ...
  def generateContainerTrait(ctx : GenerationContext, packageGenDir: String, packElement: EPackage) {
    ...
    val pr = new PrintWriter(new File(packageGenDir + "/" + formatedFactoryName + ".kt"),"utf-8")
    ...
    //generate getter
    pr.println("\tfun eContainer = internal_eContainer")

    //generate setter
    pr.print("\n\tfun setEContainer( container : " + formatedFactoryName + ", unsetCmd : Option[()=>Any] ) {\n")
