...
class RootLoader(ctx : GenerationContext, genDir: String, modelingPackage: EPackage) {
  ...
  private def generateLoadMethod(pr: PrintWriter, elementType: EClass) {
    ...
    pr.println("def loadModel(str: String) : scala.Option[" + ProcessorHelper.fqn(ctx,elementType) + "] = {")
    ...
    pr.println("")
    pr.println("def loadModel(file: File) : scala.Option[" + ProcessorHelper.fqn(ctx,elementType) + "] = {")
    pr.println("loadModel(new FileInputStream(file))")
    pr.println("}")

    pr.println("")
    pr.println("def loadModel(is: InputStream) : scala.Option[" + ProcessorHelper.fqn(ctx,elementType) + "] = {")
