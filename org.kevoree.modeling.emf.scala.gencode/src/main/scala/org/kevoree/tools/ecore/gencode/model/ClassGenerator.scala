    ...
    cls.getEReferences.foreach {
      ref =>
        ...
        if (ref.getUpperBound == -1) {
          // multiple values
          pr.println("\t\tprivate lazy val " + protectReservedWords(ref.getName) + " : scala.collection.mutable.ListBuffer[" + typeRefName + "] = new scala.collection.mutable.ListBuffer[" + typeRefName + "]()\n")
