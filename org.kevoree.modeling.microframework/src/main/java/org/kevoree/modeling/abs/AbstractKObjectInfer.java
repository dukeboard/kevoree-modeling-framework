...
public abstract class AbstractKObjectInfer extends AbstractKObject implements KInfer {
    ...
    public KInferState readOnlyState() {
        KMemorySegment raw = _manager.segment(_universe,_time,_uuid, AccessMode.RESOLVE,metaClass(), null);
        if (raw != null) {
    ...
    public KInferState modifyState() {
        KMemorySegment raw = _manager.segment(_universe,_time,_uuid, AccessMode.NEW,metaClass(), null);
        if (raw != null) {
            if (raw.get(KMetaInferClass.getInstance().getCache().index(), metaClass()) == null) {
                internal_load(raw);
            }
