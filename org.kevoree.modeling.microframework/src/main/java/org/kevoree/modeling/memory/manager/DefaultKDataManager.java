...
import org.kevoree.modeling.scheduler.DirectScheduler;
...
import org.kevoree.modeling.util.DefaultOperationManager;
...
public class DefaultKDataManager implements KDataManager {
    ...
    public void initKObject(KObject obj) {
        ...
        OOKLongTree timeTree = new OOKLongTree();
        timeTree.inc();
    ...
    public void getRoot(long universe, long time, final Callback<KObject> callback) {
        bumpKeyToCache(KContentKey.createRootUniverseTree(), new Callback<KCacheElement>() {
            @Override
            public void on(KCacheElement rootGlobalUniverseIndex) {
                if (rootGlobalUniverseIndex == null) {
                    ...
                    bumpKeyToCache(universeTreeRootKey, new Callback<KCacheElement>() {
                        @Override
                        public void on(KCacheElement universeTree) {
                            if (universeTree == null) {
                                callback.on(null);
                            } else {
                                long resolvedVal = ((OOKLongLongTree) universeTree).previousOrEqualValue(time);
    ...
    public void setRoot(final KObject newRoot, final Callback<Throwable> callback) {
        bumpKeyToCache(KContentKey.createRootUniverseTree(), new Callback<KCacheElement>() {
            @Override
            public void on(KCacheElement globalRootTree) {
                ...
                if (closestUniverse != newRoot.universe()) {
                    KLongLongTree newTimeTree = new OOKLongLongTree();
                    ...
                    _cache.put(universeTreeRootKey.universe, universeTreeRootKey.time, universeTreeRootKey.obj, (KCacheElement) newTimeTree);
                    if (callback != null) {
                        callback.on(null);
                    }
                } else {
                    ...
                    bumpKeyToCache(universeTreeRootKey, new Callback<KCacheElement>() {
                        @Override
                        public void on(KCacheElement resolvedRootTimeTree) {
                            KLongLongTree initializedTree = (KLongLongTree) resolvedRootTimeTree;
                            if (initializedTree == null) {
                                initializedTree = new OOKLongLongTree();
                                _cache.put(universeTreeRootKey.universe, universeTreeRootKey.time, universeTreeRootKey.obj, (KCacheElement) initializedTree);
                            }
    ...
    public void reload(KContentKey[] keys, final Callback<Throwable> callback) {
        ...
        for (int i = 0; i < keys.length; i++) {
            KCacheElement cached = _cache.get(keys[i].universe, keys[i].time, keys[i].obj);
                ...
                if (error != null) {
                    ...
                    for (int i = 0; i < strings.length; i++) {
                        if (strings[i] != null) {
                            ...
                            KCacheElement cachedObj = _cache.get(correspondingKey.universe, correspondingKey.time, correspondingKey.obj);
    ...
    public void bumpKeyToCache(final KContentKey contentKey, final Callback<KCacheElement> callback) {
        KCacheElement cached = _cache.get(contentKey.universe, contentKey.time, contentKey.obj);
        if (cached != null) {
            callback.on(cached);
        } else {
            ...
            _db.get(keys, new ThrowableCallback<String[]>() {
                ...
                public void on(String[] strings, Throwable error) {
                    if (strings[0] != null) {
                        KCacheElement newObject = internal_unserialize(contentKey, strings[0]);
                        if (newObject != null) {
    ...
    public void bumpKeysToCache(KContentKey[] contentKeys, final Callback<KCacheElement[]> callback) {
        boolean[] toLoadIndexes = null;
        int nbElem = 0;
        final KCacheElement[] result = new KCacheElement[contentKeys.length];
                ...
                public void on(String[] payloads, Throwable error) {
                    for (int i = 0; i < payloads.length; i++) {
                        if (payloads[i] != null) {
                            ...
                            KCacheElement newObject = internal_unserialize(newObjKey, payloads[i]);
                            if (newObject != null) {
    ...
    private KCacheElement internal_unserialize(KContentKey key, String payload) {
        KCacheElement result;
        boolean isUniverseNotNull = key.universe != KConfig.NULL_LONG;
        if (KConfig.END_OF_TIME == key.obj) {
            if (isUniverseNotNull) {
                result = new OOKLongLongTree();
            } else {
